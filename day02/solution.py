...
def part1(data):
    # part 1 - with regex
    # find occurrences of the key character in pword string,
    # ensuring that it is in between the min / max amount set
    valids = 0
    for line in data:
        character, pword, o_min, o_max = get_splits(line)
        occurrences = len(re.findall(character, pword))
        valid = occurrences >= o_min and occurrences <= o_max
        valids += valid
        # print(character,o_min,o_max,pword, occurrences)
    return valids
